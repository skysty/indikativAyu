<?php
		...
		if(isset($_POST['confirmed'])){
            ...
            $kod_korsetid = $_POST['$kod_korsetid'];
			...
			$main_sql = mysqli_query($connection,"SELECT korsetkishter.ball, korsetkishter.kod_korsetkish,
			engbekter.engbekID, engbekter.sani, engbekter.kod_fakul, engbekter.divBall, engbekter.univ_avtor_san, 
			engbekter.kod_kizm FROM engbekter INNER JOIN korsetkishter ON engbekter.kod_korset = korsetkishter.kod_korsetkish WHERE del=0 and engbekID = '$engbek_id'") or die(mysqli_error($connection)); 
			...
			if($status == 3){
				...
				if($avtor_san > 1){				
					...
					if($kod_korsetkish == $kod_korsetid){
