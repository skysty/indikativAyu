	...
	<script>
		...
		$(document).ready(function() {
			$("#status").change(function() {
				...
				var lang = ER_Locale.lang;
				if(lang==='kaz'){
					message=kod_stat + ' статусын таңдадыңыз!!!';
				}else if(lang==='rus'){
					message='Вы выбрали статус ' + kod_stat;
				}
				$.ajax({
					method: "POST",
	...
	<div class = "content">
		...
		<div class = "content_wrapper" style = "width: 100%; margin: 0 auto; margin-top: 10px;">
			...
			<div class = "engbek">
				...
				<form id="formconf" action = "confirmation.php" method = "post">
					...
					<table>
						...
						<tr>
							...
							<td><?php echo ($_SESSION['lang'] == 'tr') ? $engbek['facultyNameTR'] : $engbek['facultyNameKZ']; ?>
								</span>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								...
								<?php 
										if ($_SESSION['lang'] == 'tr') {
											echo $engbek['cafedraNameTR'] ?? 'Default Value';
										} else {
											echo $engbek['cafedraNameKZ'] ?? 'Default Value';
										}
									?>
								</span>
							</td>
						...
						<tr>
							...
							<td><span><strong><?=$oL::get('Көрсеткіш атауы')?>:</strong></td><td><?php echo ($_SESSION['lang'] == 'tr')? $engbek['korsetkish_ati2']: $engbek['korsetkish_ati']?></span></td>
...
<script>
		...
		// Listen for form submission
			$('#formconf').submit(function(event) {
				...
				Swal.fire({
					title: ER_Locale.get('Оқытушы еңбегін растайсыз ба?'),
					icon: 'warning',
					showCancelButton: true,
					confirmButtonText: ER_Locale.get('Иә, растаймын!'),
