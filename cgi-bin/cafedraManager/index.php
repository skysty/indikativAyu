			...
			<div class = 'tutors_table'>
				...
				<?php
					...
					while($tutor2 = mysqli_fetch_array($sql_tuts)){
						...
						echo "<tr><td>".$i."</td><td>".$sLastName." ".$sFirstName."</td><td>" . $sum_val . "</td><td><a href = \"tolyk.php?ID=" . $tutor2['TutorID'] . "\">".$oL::get('Толық')." >></a></td></tr>";
			...
			?>
					...
					<div class = "univer_table">
						...
						//###########################################univer table display###############################################
							...
							$sql3 = mysqli_query($connection, "SELECT engbekter.kod_fakul, SUM(engbekter.ball) AS sum_univer FROM engbekter 
							...
							$sql5 = mysqli_query($connection, "SELECT engbekter.kod_fakul, SUM(engbekter.ball*0.90)*0.35 AS sum_univer_oku, korsetkishter.typeID FROM engbekter INNER JOIN faculties ON faculties.FacultyID = engbekter.kod_fakul INNER JOIN korsetkishter ON korsetkishter.kod_korsetkish = engbekter.kod_korset WHERE korsetkishter.typeID = 2") or die(mysqli_error($connection));
							...
							$sql6 = mysqli_query($connection, "SELECT engbekter.kod_fakul, SUM(engbekter.ball*0.90)*0.15 AS sum_univer_tarbie, korsetkishter.typeID FROM engbekter INNER JOIN faculties ON faculties.FacultyID = engbekter.kod_fakul INNER JOIN korsetkishter ON korsetkishter.kod_korsetkish = engbekter.kod_korset WHERE korsetkishter.typeID = 3") or die(mysqli_error($connection));
