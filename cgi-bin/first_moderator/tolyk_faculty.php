	...
	<div class = "content">
		<div class = "content_wrapper" style = "width: 100%; margin: 0 auto; margin-top: 10px;">		
			<?php
					...
					engbekter.ball, 
					faculties.facultyNameKZ, 
					cafedras.cafedraNameKZ, korsetkishter.korsetkish_ati, faculties.FacultyID, korsetkishter.typeID
					FROM engbekter
					INNER JOIN faculties ON faculties.FacultyID = engbekter.kod_fakul
					INNER JOIN cafedras ON cafedras.cafedraID = engbekter.kod_kafedra
					INNER JOIN korsetkishter ON korsetkishter.kod_korsetkish = engbekter.kod_korset
					...
					INNER JOIN cafedras ON cafedras.cafedraID = engbekter.kod_kafedra
					INNER JOIN korsetkishter ON korsetkishter.kod_korsetkish = engbekter.kod_korset
					WHERE kod_fakul = '$_GET[ID]' AND korsetkishter.typeID = 4") or die(mysqli_error($connection));
					
					$faculty5 = mysqli_fetch_array($sql6);

					$a = array($faculty4['sum_tarbie'],$faculty2['sum_gylym'],$faculty3['sum_oku'],$faculty5['sum_juldyzdyk']);
					$univ = array_sum($a);
                    ...
                    if ($_SESSION['lang'] != 'kaz'){
                        ...
                        $sFacult = isset($faculty['facultyNameRU']) && mb_strlen($faculty['facultyNameRU']) ? $faculty['facultyNameRU'] : $faculty['facultyNameKZ'];
                    }
			...
			<div class = "engbek">
				<form action = "confirmation.php" method = "post">
					<table style = "font-size: 19px;">
						...
						<tr>
							<td>
								<strong><?= $oL::get('Жұлдыздық кретерийі')?> 10%:</strong>
							</td>
							<td>
								<?php 
									$format='%.2f/%.2f'; 
									echo sprintf($format,$faculty5['sum_juldyzdyk1'],$faculty5['sum_juldyzdyk']); 
								?>
							</td>
						</tr>
						<tr>
