	...
	<div class = "content">
		<div class = "content_wrapper" style = "width: 100%; margin: 0 auto; margin-top: 10px;">
			...
			<?php
				...
				engbekter.engbekID, 
				SUM(engbekter.ball)*0.10 AS sum_juldyzyk,
				SUM(engbekter.ball) AS sum_juldyzyk1, 
				engbekter.file_ati, 
				engbekter.sani, 
				cafedras.cafedraNameKZ, 
				cafedras.cafedraNameRU, 
				...
				INNER JOIN cafedras ON cafedras.cafedraID = engbekter.kod_kafedra
				INNER JOIN korsetkishter ON korsetkishter.kod_korsetkish = engbekter.kod_korset
				WHERE kod_kafedra = '$_GET[ID]' AND korsetkishter.typeID = 4") or die(mysqli_error($connection));
				
				$cafedra5 = mysqli_fetch_array($sql6);
				/*data */
				$a = array($cafedra4['sum_tarbie'],$cafedra2['sum_gylym'],$cafedra3['sum_oku'],$cafedra5['sum_juldyzyk']);
				$univ = array_sum($a);
			...
			<div class = "engbek">
				<form action = "confirmation.php" method = "post">
					<table style = "font-size: 19px;">
						...
						<tr>
							<td>
								<strong><?= $oL::get('Әлеуметтік-мәдени бағыты')?> 10%:</strong>
							</td>
							<td>
								<?php 
										$format='%.2f/%.2f'; echo sprintf($format,$cafedra5['sum_juldyzyk1'],$cafedra5['sum_juldyzyk']); 
								?>
							</td>
						</tr>
						<tr>
