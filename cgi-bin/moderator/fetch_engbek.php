...
/*	$sql = "SELECT * FROM state WHERE countryID = '".$_POST["countryID"]."' AND stateID = '".$_POST["stateID"]."'";
	...
	$where .= (!empty($_POST["FacultyID"])) ? ' AND kod_fakul = "' . $_POST["FacultyID"] . '"' : '';
	$where .= (!empty($_POST["statusID"])) ? ' AND kod_stat = "' . $_POST["statusID"] . '"' : '';
	$where .= ' AND del=0 ';
	...
	while($row = mysqli_fetch_array($result)){
		...
		$sLastName = $row['lastname'];
			...
			echo "<td><a target='_blank' href = \"engbek.php?ID=" . $row['engbekID'] . "\">".$oL::get('Тексеру')."</a></td>";
			echo "</tr>";
			$i++;
		}
