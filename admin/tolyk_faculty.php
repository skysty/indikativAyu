	...
	<div class = "content">
		<div class = "content_wrapper" style = "width: 100%; margin: 0 auto; margin-top: 10px;">		
			<?php
					...
					$univ = array_sum($a);
					$sql2 = mysqli_query($connection,"SELECT engbekter.engbekID, 
					SUM(engbekter.ball) AS sum_val, 
					IFNULL($univ,0)/IFNULL(faculties.shtat_sany, 1) AS avg_faculty, 
					engbekter.file_ati, 
					engbekter.sani, 
					engbekter.univ_avtor_san, 
					engbekter.eskertu, 
			...
			<div class = "works">
					<?php
						...
						WHERE faculties.FacultyID = '$_GET[ID]' AND engbekter.univ_avtor_san IS NULL AND  engbekter.kod_kafedra  is null ORDER BY engbekter.engbekID DESC";
						
						$result = mysqli_query($connection,$sql) or die(mysqli_error($connection));
						
						echo "<table><tr><th>№</th><th>".$oL::get('Факультет/ҒЗО')."</th><th>".$oL::get('Аты жөні')."</th><th>".$oL::get('Көрсеткіш')."</th><th>".$oL::get('Саны')."</th><th>".$oL::get('Автор саны')."</th><th>".$oL::get('Файл аты')."</th><th>".$oL::get('Балл')."</th><th>".$oL::get('Қайтару себебі')."</th><th>".$oL::get('Статус')."</th></tr>";
						...
						while($row = mysqli_fetch_array($result)){
                            ...
                            if ($_SESSION['lang'] != 'kaz'){
                                ...
                                $sFaculty = isset($row['facultyNameTR']) && mb_strlen($row['facultyNameTR']) ? $row['facultyNameTR'] : $row['facultyNameKZ'];
							...
							echo "<tr><td>".$i."</td><td>".$sFaculty."</td><td>".$sLastName." ".$sFirstName."</td><td>".$row["korsetkish_ati"]."</td><td>".$row["sani"]."</td><td>".$row["univ_avtor_san"]."</td><td><a target='_blank' href = " .$row['file_ati'] .">".$row["file_ati"]."</a></td><td>".$row["ball"]."</td><td><b>".$sebep."</b><br>".$row["kayt_sebeb"]."</td><td>".$statuss."</td></tr>";
							$i++;
						}
