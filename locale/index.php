	...
	<div class = "content">
		<div class = "content_wrapper" style = "width: 100%; margin: 0 auto; margin-top: 5px;">
			...
			<div class = "shagymTable">
				...
				<table>
					<tr>
						<th>№</th><th>Қазақша</th><th>Түрікше</th><th>Операция</th>
					</tr>
					<?php

						$query = mysqli_query($connection,"SELECT * FROM locale") or die(mysqli_error($connection));
						while($korsetkish = mysqli_fetch_array($query)){
							echo "<tr id='". $korsetkish['id'] ."'><td>" . $korsetkish['id'] ."</td><td>".$korsetkish['kaz']."</td><td>".$korsetkish['tr']."</td><td style='width: 13%'><div><span><a href=\"edit.php?id=".$korsetkish['id']."\">".$oL::get('Өңдеу')."</a></span><span style='float: right'><a href=\"deleted.php?id=".$korsetkish['id']."\">".$oL::get('Өшіру')."</a></span></div></td></tr>";
						}
					?>
				</table>
			</div>
